...
class pfd(object):
    ...
    def greyscale_1(self, array2d, **kwargs):
        """
        greyscale(array2d, **kwargs):
            Plot a 2D array as a greyscale image using the same scalings
                as in prepfold.
    ...
    def plot_intervals(self, phasebins='All', device='/xwin'):
        ...
        profs = self.greyscale_1(profs, rangex=[lo, hi], rangey=[0.0, self.npart],
    ...
    def plot_subbands(self, phasebins='All', device='/xwin'):
        ...
        profs = self.greyscale_1(profs, rangex=[lo, hi], rangey=[0.0, self.nsub],
    ...
    def DM_curve(self):
        ...
        for ii, DM in enumerate(DMs):
            ...
            if interp:
                ...
                for jj in range(self.nsub):
                    ...
                    if not delay_bins==0:
                        profs[jj] = Num.concatenate((profs[jj][delay_bins:], profs[jj][:delay_bins]))
                subdelays_bins += new_subdelays_bins
                avgprof = self.avgprof
            sumprof = profs.sum(0)
            chis[ii] = self.calc_redchi2(prof=sumprof, avg=avgprof)
        ...
        chis = chis - Num.min(chis)
        max_chis = Num.max(chis) 
        if max_chis != 0:
            chis = chis / max_chis
...
if __name__ == "__main__":
    ...
    testpfd = "/home/cnn/AIwork/problem/G31.27+4.58_20191009_snapshot-M09-P2_DM950.50_25.26ms_Cand.pfd"
