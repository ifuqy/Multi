...
def load_data_from_config(config_path):
    ...
    if use_dataloader:
        ...
        files_exist = all(os.path.exists(paths.get(k, "")) for k in ["TRAIN", "VAL", "TEST"])

        if files_exist:
            print("Loading from dataloader .pkl files...")
            train = load_from_pickle(paths["TRAIN"])
            val = load_from_pickle(paths["VAL"])
            if use_test:
    ...
    if use_dataset:
        ...
        X_train, X_val, y_train, y_val = train_test_split(
            train_pfds, train_target, test_size=val_size, random_state=random_state
        )
        ...
        val_loader = multi_AI.create_Combined_dataloader(X_val, y_val, batch_size=config["TRAIN"]["BATCH_SIZE"])
        if use_test:
        ...
        if save_pickle:
            ...
            save_to_pickle(val_loader, os.path.join(save_path, "val_loader.pkl"))

            save_pfd_label(X_train, y_train,os.path.join(save_path, 'train_labels.txt'))
            save_pfd_label(X_val, y_val, os.path.join(save_path, 'val_labels.txt'))
...
def main():
    config_path = "./config/train_cfg.yaml"
    config = load_config(config_path)
    ...
    train_dataloader, val_dataloader, test_dataloader = load_data_from_config(config_path)
    ...
    print(f"Validation data Shape: {len(val_dataloader[0][0][0]) * (len(val_dataloader) - 1) + len(val_dataloader[len(val_dataloader)-1][0][0])}")
    ...
    multi_AI.fit_dataloader(train_dataloader, val_dataloader, test_dataloader)
